...
const Friend = ({ user }: { user: any }) => {
  ...
  if (amISender) {
    ...
    icon =
      lastMessageType === "text" ? (
        ...
        <MdCheckCircleOutline size={"16px"} className="text-red-500" />
      );
  } else {
    if (!lastMessage) {
  ...
  return (
    <Link
      href={`/chat/${user._id}`}
      className="flex items-center justify-between border-b-2 border-[#E3E6E8] rounded-sm my-2 p-3"
    >
      <div className="flex gap-2">
        ...
        <div>
          ...
          <p className={`${messageStatus === "New Snap" ? 'text-purple-600' : null} flex gap-1 text-xs font-bold text-gray-500`}>
            <span
              className={`${
                messageStatus === "New Snap" ? "hidden" : null
              }`}
            >
              {icon}
            </span>
